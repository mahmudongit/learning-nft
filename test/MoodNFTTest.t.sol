//SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import {console, Test} from "forge-std/Test.sol";
import {MoodNFT} from "../src/MoodNFT.sol";
import {Base64} from "@openzeppelin/contracts/utils/Base64.sol";

contract MoodNFTTest is Test {
    MoodNFT moodNft;
    string public constant HAPPY_SVG_URI = "data:image/svg+xml;base64,";
    string public constant SAD_SVG_URI = "data:image/svg+xml;base64,";

    address USER = makeAddr("USER");

    function setUp() public {
        moodNft = new MoodNFT(HAPPY_SVG_URI, SAD_SVG_URI);
    }

    function testFlipMoodIntegration() public {
        vm.prank(USER);
        moodNft.mintNft(); // Starts as SAD per your mintNft()
        
        vm.prank(USER);
        moodNft.flipMoodNft(0); // Flip to HAPPY

        string memory actualTokenUri = moodNft.tokenURI(0);
        
        // Use HAPPY_SVG_URI because we flipped it
        string memory expectedTokenUri = _getExpectedJSON("happy", HAPPY_SVG_URI);

        assertEq(actualTokenUri, expectedTokenUri);
    }

    function _getExpectedJSON(string memory moodValue, string memory imageUri) internal view returns (string memory) {
        // Matches the string.concat in your MoodNFT.sol exactly
        string memory json = string.concat(
            '{"name":"', moodNft.name(), '", "description":"An NFT that changes to the mood of its owner", "attributes":[{"trait_type":"mood", "value":"', moodValue, '"}], "image":"', imageUri, '"}'
        );
        
        return string(
            abi.encodePacked(
                "data:application/json;base64,",
                Base64.encode(bytes(json))
            )
        );
    }
}
